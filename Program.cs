...
namespace Weather_Report
{
    ...
    public class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            ...
            int[] availableRequests = { 1, 2, 3 };   //// COULD BE ENUM
            ...
            while (availableRequests.Contains(request) == false)  //// COULD BE SWITCH CASE
            {
                ...
                if (request == 1)
                {
                    var averageTemp = AvarageTempFinder();
                ...
                if (request == 2)
                {
                    var totalPrecipitation = TotalPrecipitation();
        ...
        {
            // The string here is our endpoint, where paramter 1 is the current temp measured once per hour
            ...
            var stations = JsonConvert.DeserializeObject<Stations>(data.Result);
            ...
            foreach (var station in stations.station)
            {
                if (station.value != null)
                {
                    ...
                    var temperature = double.Parse(temperatureStr, format);
        ...
        {
            // The string here is our endpoint where parameter 23 is totalt precipation for each of the last for months and station 53430 is Lund
            var data = ApiService.GetDataAsync("version/latest/parameter/23/station/53430/period/latest-months/data.json");

            // Converts our Json string to an object
            var precipitation = JsonConvert.DeserializeObject<Precipitation>(data.Result);

            var totalPrecipitation = 0.0;

            foreach (var value in precipitation.value)
            {
                var precipitationStr = value.value;
                var precipitationParsed = double.Parse(precipitationStr, CultureInfo.InvariantCulture);
        ...
        private static async Task CurrentTemperature()
        {
        ...
        private static Task GetTemp(Stations stations, CancellationToken cancellationToken)
        {
            ...
            var task = Task.Run(() =>
           {
               foreach (var item in stations.station)
               {
                   ...
                   var station = JsonConvert.DeserializeObject<StationCurrentTemp>(stationsStr.Result);

                   var latest = station.period.ToList().Find(x => x.key == "latest-hour");
